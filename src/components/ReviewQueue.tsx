...
import { Brain, RefreshCw, Sparkles, PartyPopper } from 'lucide-react';
...
export default function ReviewQueue({ cards, onUpdateCards }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 min-h-[240px] flex flex-col">
        ...
        {!revealed ? (
          ...
          <div className="mt-4 flex-1 flex flex-col space-y-3">
            ...
            {gradingBusy && (
              <p className="text-[11px] text-slate-400 flex items-center gap-1.5"><RefreshCw className="w-3 h-3 animate-spin" /> Comparing your explanation…</p>
            )}
            {aiFeedback && (
              <div className="bg-focus-primary/5 border border-focus-primary/20 rounded-xl p-3 flex items-start gap-2">
